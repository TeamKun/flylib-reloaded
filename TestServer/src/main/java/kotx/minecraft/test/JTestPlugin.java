...
class JMenuCommand extends Command {
    ...
    public void execute(@NotNull CommandContext context) {
        ChestMenu.display(context.getPlayer(), builder -> {
            builder.item(5, 1, ExtensionsKt.item(Material.DIAMOND, itemBuilder -> itemBuilder
                            .displayName("Super Diamond")
                            .lore("Very Expensive")
                            .enchant(Enchantment.LUCK)
                            .flag(ItemFlag.HIDE_ENCHANTS)),
                    event -> context.send(componentBuilder -> TextExtensionsKt.append(componentBuilder, "You clicked me!?", TextDecoration.BOLD)));
        });
    }
}